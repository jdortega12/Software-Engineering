...
func Test_CreateManagerRequest_ValidRequest(t *testing.T) {
	...
	router.POST("/test", func(ctx *gin.Context) {
		setSessionUser(ctx, "MeytonPanning", "testing")
	...
	jsonData, _ := json.Marshal(managerRequest)
...
func Test_CreateManagerRequest_NoSession(t *testing.T) {
	router := setupTestRouter()
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/api/v1/createManagerRequest", nil)
	router.ServeHTTP(w, req)
...
func Test_CreateManagerRequest_InvalidSender(t *testing.T) {
	...
	router.POST("/test", func(ctx *gin.Context) {
		...
		handleCreateManagerRequest(ctx)
	})
...
func Test_handleCreateManagerRequest_BadJSON(t *testing.T) {
	initTestDB()
	router := setupTestRouter()
...
func Test_handleCreateManagerRequest_InvalidRequest(t *testing.T) {
	initTestDB()
	router := setupTestRouter()
...
func TestRemovePLayerValid(t *testing.T) {
	//Add player and manager to DB
	player := &model.User{
		Username: "brady",
		Password: "wasspord",
	...
	router := setupTestRouter(func(ctx *gin.Context) {
		setSessionUser(ctx, "manny", "passwass")
		ctx.Next()
	})
	...
	// mock request
	w := sendMockHTTPRequest(http.MethodPost, "/api/v1/removePlayer", bytes.NewBuffer(jsonData), router)
